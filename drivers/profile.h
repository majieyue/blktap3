...
static inline void
tp_open(struct profile_info *prof, char *tap_name, char *log_name,
        int size)
{
...
static inline void tp_close(struct profile_info *prof)
{
    int i;
    struct profile_times *pt;
...
static inline u64 tp_get_id(struct profile_info *prof)
{
    return prof->seq++;
}

static inline int tp_fn_id(struct profile_info *prof, const char *name)
{
    int i;
    struct profile_times *pt;
...
static inline void __tp_in(struct profile_info *prof, const char *func)
{
    long long _time;
...
static inline void __tp_out(struct profile_info *prof, const char *func)
{
    long long _time;
    int idx = tp_fn_id(prof, func);
    struct profile_times *pt = &prof->pt[idx];
...
static inline void
__tp_log(struct profile_info *prof, u64 id, const char *func,
         int direction)
{
    long long _time;
    asm volatile (".byte 0x0f, 0x31":"=A" (_time));
