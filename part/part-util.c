...
void partition_table_dump(struct partition_table *pt)
{
    int i;
...
static int dump_partitions(const char *image)
{
    int fd, ret;
...
static void __dump_signature(struct partition_table *pt, int part)
{
    if (part < 1 || part > 4)
        errno = EINVAL;
    else {
...
static int dump_signature(const char *image, int part)
{
    int fd, ret;
    struct partition_table pt;
...
static int count_partitions(const char *image, int *count)
{
    int i, fd, ret;
    struct partition_table pt;
