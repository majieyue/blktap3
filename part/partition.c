...
void primary_partition_in(struct primary_partition *p)
{
    p->lba = le32_to_cpu(p->lba);
    p->blocks = le32_to_cpu(p->blocks);
}

void primary_partition_out(struct primary_partition *p)
{
    p->lba = cpu_to_le32(p->lba);
    p->blocks = cpu_to_le32(p->blocks);
}

void partition_table_in(struct partition_table *pt)
{
    int i;
...
void partition_table_out(struct partition_table *pt)
{
    int i;
...
int primary_partition_validate(struct primary_partition *p)
{
...
int partition_table_validate(struct partition_table *pt)
{
    int i;
