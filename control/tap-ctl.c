...
static void tap_cli_list_usage(FILE * stream)
{
    fprintf(stream,
            "usage: list [-h] [-p pid] [-m minor] [-t type] [-f file]\n");
}

static void tap_cli_list_row(tap_list_t * entry)
{
    char minor_str[10] = "-";
    char state_str[10] = "-";
    char pid_str[10] = "-";
...
static void tap_cli_list_dict(tap_list_t * entry)
{
    int d = 0;
...
int tap_cli_list(int argc, char **argv)
{
...
static void tap_cli_create_usage(FILE * stream)
{
...
static int tap_cli_create(int argc, char **argv)
{
...
static void tap_cli_destroy_usage(FILE * stream)
{
    fprintf(stream, "usage: destroy <-p pid> <-m minor>\n");
}

static struct timeval *tap_cli_timeout(const char *optarg)
{
...
static int tap_cli_destroy(int argc, char **argv)
{
    int c, pid, minor;
...
static void tap_cli_spawn_usage(FILE * stream)
{
    fprintf(stream, "usage: spawn\n");
}

static int tap_cli_spawn(int argc, char **argv)
{
    int c, tty;
    pid_t pid;
...
static void tap_cli_attach_usage(FILE * stream)
{
    fprintf(stream, "usage: attach <-p pid> <-m minor>\n");
}

static int tap_cli_attach(int argc, char **argv)
{
    int c, pid, minor;
...
static void tap_cli_detach_usage(FILE * stream)
{
    fprintf(stream, "usage: detach <-p pid> <-m minor>\n");
}

static int tap_cli_detach(int argc, char **argv)
{
    int c, pid, minor;
...
static void tap_cli_close_usage(FILE * stream)
{
    fprintf(stream, "usage: close <-p pid> <-m minor> [-f force]\n");
}

static int tap_cli_close(int argc, char **argv)
{
...
static void tap_cli_pause_usage(FILE * stream)
{
    fprintf(stream, "usage: pause <-p pid> <-m minor>\n");
}

static int tap_cli_pause(int argc, char **argv)
{
    int c, pid, minor;
    struct timeval *timeout;
...
static void tap_cli_unpause_usage(FILE * stream)
{
    fprintf(stream, "usage: unpause <-p pid> <-m minor> [-a args]\n");
}

int tap_cli_unpause(int argc, char **argv)
{
    const char *args;
    int c, pid, minor;
...
static void tap_cli_major_usage(FILE * stream)
{
    fprintf(stream, "usage: major [-h]\n");
}

static void tap_cli_open_usage(FILE * stream)
{
    fprintf(stream,
            "usage: open <-p pid> <-m minor> <-a args> [-R readonly] "
            "[-e <minor> stack on existing tapdisk for the parent chain] "
            "[-r turn on read caching into leaf node] [-2 <path> "
            "use secondary image (in mirror mode if no -s)] [-s "
            "fail over to the secondary image on ENOSPC]\n");
...
static int tap_cli_open(int argc, char **argv)
{
...
static void tap_cli_stats_usage(FILE * stream)
{
    fprintf(stream, "usage: stats <-p pid> <-m minor>\n");
}

static int tap_cli_stats(int argc, char **argv)
{
    pid_t pid;
    int c, minor, err;
