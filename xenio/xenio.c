...
__printf(1, 2) static char* mprintf(const char *fmt, ...)
{
	va_list ap;
	char *s;
...
__printf(3, 4) static bool xenio_xs_exists(struct xs_handle *xs,
 ...
 */
static inline int xenio_device_vscanf(xenio_device_t *device, const char *path,
		const char *fmt, va_list ap)
{
	char *s;
	int n;
 ...
 */
__scanf(3, 4) static int xenio_device_scanf(xenio_device_t *device,
...
__scanf(3, 4) static int xenio_device_scanf_otherend(xenio_device_t *device,
		const char *path, const char *fmt, ...)
{
	va_list ap;
	int n;
 ...
 */
static inline int xenio_device_vprintf(xenio_device_t *device, const char *key,
		int mkread, const char *fmt, va_list ap)
{
	xenio_backend_t *backend = device->backend;
	...
	path = mprintf("%s/%d/%s/%s", backend->path, device->domid, device->name,
			key);
	if (!path) {
		err = -errno;
		goto fail;
 ...
 */
__printf(4, 5) static int xenio_device_printf(xenio_device_t *device,
 ...
 */
static inline int xenio_device_watch_frontend_state(xenio_device_t *device)
{
	xenio_backend_t *backend = device->backend;
	bool nerr;
	int err;
...
static int xenio_device_check_frontend_state(xenio_device_t *device)
{
	...
	s = xenio_xs_read(backend->xs, backend->xst, device->frontend_state_path);
	if (!s) {
		err = -errno;
		goto fail;
	}
 ...
 */
static inline int xenio_backend_handle_otherend_watch(xenio_backend_t *backend,
		char *path)
{
	xenio_device_t *device;
	int err = 0;
 ...
 * TODO Only called by xenio_backend_read_watch.
 */
static inline int xenio_backend_handle_backend_watch(xenio_backend_t *backend,
		char *path)
{
	char *s, *end, *name;
	int domid;
...
xenio_backend_t *xenio_backend_create(const char *name,
		const struct xenio_backend_ops *ops, int max_ring_page_order)
{
 ...
 */
static int blkback_probe(xenio_device_t *xbdev, domid_t domid, const char *name)
{
