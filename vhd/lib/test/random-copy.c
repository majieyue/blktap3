...
static int random_copy_carve_source(struct random_copy_ctx *ctx)
{
    int err, i, n;
    struct stat64 st;
    off64_t bytes, start;
...
static int random_copy_permute_source(struct random_copy_ctx *ctx)
{
    int i;
...
static int
random_copy_init(struct random_copy_ctx *ctx, const char *src,
                 const char *dst)
{
    int err;
...
static int random_copy(struct random_copy_ctx *ctx)
{
    char *buf;
    int i, err;
...
static void random_copy_close(struct random_copy_ctx *ctx)
{
    close(ctx->sfd);
    close(ctx->dfd);
    free(ctx->chunks);
