 ...
 */
static tap_list_t *_tap_list_alloc(void)
{
...
static void _tap_list_free(tap_list_t * tl, struct tqh_tap_list *list)
{
    if (tl->type) {
        free(tl->type);
        tl->type = NULL;
...
int _parse_params(const char *params, char **type, char **path)
{
    char *ptr;
    size_t len;
...
static int _tap_ctl_find_minors(struct tqh_tap_list *list)
{
...
int _tap_ctl_find_tapdisks(struct tqh_tap_list *list)
{
    const char *pattern, *format;
    glob_t glbuf = { 0 };
    int err, i, n_taps = 0;
...
static int _tap_ctl_list_tapdisk(pid_t pid, struct tqh_tap_list *list)
{
