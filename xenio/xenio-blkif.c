                              ...
                              xenio_blkif_req_t * req)
{
    ...
    if (req->n_segs < 0 || req->n_segs > BLKIF_MAX_SEGMENTS_PER_REQUEST)
        goto fail;
...
int xenio_blkif_get_requests(xenio_blkif_t * blkif,
                             blkif_request_t ** reqs, int count, int final)
{
    blkif_common_back_ring_t *ring = &blkif->rings.common;
    int n = 0, work;
...
static inline blkif_response_t *xenio_blkif_get_response(xenio_blkif_t *
                                                         blkif,
                                                         RING_IDX rp)
{
    blkif_back_rings_t *rings = &blkif->rings;
...
static void xenio_blkif_ring_unmap(xenio_blkif_t * blkif)
{
    ...
    if (sring) {
        xc_gnttab_munmap(ctx->xcg_handle, sring, blkif->ring_n_pages);
